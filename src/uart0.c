...
void UART0_Init(uint32_t baudRate, uint32_t sysClk)
{
    ...
    GPIO_PORTA_AFSEL_R |= (1 << 0) | (1 << 1); // Alternate function
    ...
    // Calculate baud rate
    float brd = (float)sysClk / (16.0f * baudRate);
    uint32_t ibrd = (uint32_t)brd;
    uint32_t fbrd = (uint32_t)((brd - ibrd) * 64 + 0.5f);

    UART0_IBRD_R = ibrd;
    UART0_FBRD_R = fbrd;
    UART0_LCRH_R = UART_LCRH_WLEN_8 | UART_LCRH_FEN;
    UART0_CC_R = 0; // System clock
